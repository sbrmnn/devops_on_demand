class ProductPaymentProcessor
  ...
  def initialize(product)
    raise "Input must be a Product object."  unless product.class == Product
    raise "Product record must be persisted" unless product.persisted?
    @product = product
  ...
  module StripeVendor
    attr_reader :product, :merchant_id
    ...
    def settlement_currency
      self.class.adapter.supported_currencies[product.location.to_sym]
    end
    ...
    def payout_identity_missing_fields
      payout_identity = product.payout_identity
    ...
    def set_payout_identity_missing_fields(fields_needed=nil)
      ...
      SendMissingFieldsMailer.send_to_product(product).deliver
    ...
    def country(legal_entity)
      if ProductPaymentProcessor.adapter.supported_countries.keys.include?(legal_entity.address_country)
    ...
    def merchant_id
      @merchant_id ||= (product.merchant_id || add_account.id)
    end
    ...
    def add_account
      ...
      account_id = acct.id
      product.update_attribute(:merchant_id, account_id )
      acct
    end
  end
end